...
class AuthHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format, *args):      
        logger = Logger('debug', "{} - {} - {}".format(self.get_attributes()[0], self.get_attributes()[1], format % args), 'NGINX_AUTH_LOG_PATH')
        logger.write()


    def log_error(self, format, *args):
        logger = Logger('error', "{} - {} - {}".format(self.get_attributes()[0], self.get_attributes()[1], format % args), 'NGINX_AUTH_LOG_PATH')
        logger.write()
