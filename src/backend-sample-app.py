...
import sys, os, signal, base64, Cookie, cgi, urlparse, codecs
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def auth_form(self, target = None):
        ...
        html_file = codecs.open("/var/www/nginx-ldap-auth/index.html", 'r', 'utf-8')
        html = html_file.read()
        self.send_response(200)
        self.end_headers()
    ...
    def log_message(self, format, *args):
        logger = Logger('debug', "{} - {}".format(self.get_attributes()[0], format % args), 'BACKEND_LOG_PATH')
        logger.write()


    def log_error(self, format, *args):
        logger = Logger('error', "{} - {}".format(self.get_attributes()[0], format % args), 'BACKEND_LOG_PATH')
        logger.write()
...
if __name__ == '__main__':
    ...
    logger = Logger('info', "Start listening on %s:%d...\n" % Listen, 'BACKEND_LOG_PATH')
    logger.write()
